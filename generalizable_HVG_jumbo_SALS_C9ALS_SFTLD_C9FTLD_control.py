...
nano generalizable_HVG_L2_L3_ALS_jumbo_SALS_C9ALS_SFTLD_C9FTLD.sh
...
#SBATCH --job-name=generalizable_HVG_L2_L3_ALS_jumbo_SALS_C9ALS_SFTLD_C9FTLD
...
python3.8 /home/fiorini9/scratch/machine_learning_ALS/scripts/generalizable_HVG_L2_L3_ALS_jumbo_SALS_C9ALS_SFTLD_C9FTLD.py

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

nano generalizable_HVG_L2_L3_ALS_jumbo_SALS_C9ALS_SFTLD_C9FTLD.py
...
lime_file_BA4_ALS = f'/home/fiorini9/scratch/machine_learning_ALS/model_outs/LIME_LOO_optimal_delta_{par_prep}_All ALS_BA4_{par_keep_cell_type}_0_abs_case_control_narval_2.csv'
lime_file_BA9_ALS = f'/home/fiorini9/scratch/machine_learning_ALS/model_outs/LIME_LOO_optimal_delta_{par_prep}_All ALS_BA9_{par_keep_cell_type}_0_abs_case_control_narval_2.csv'
lime_file_BA4_FTLD = f'/home/fiorini9/scratch/machine_learning_ALS/model_outs/LIME_LOO_optimal_delta_{par_prep}_All FTLD_BA4_{par_keep_cell_type}_0_abs_case_control_narval_2.csv'
lime_file_BA9_FTLD = f'/home/fiorini9/scratch/machine_learning_ALS/model_outs/LIME_LOO_optimal_delta_{par_prep}_All FTLD_BA9_{par_keep_cell_type}_0_abs_case_control_narval_2.csv'

features_BA4_ALS = pd.read_csv(lime_file_BA4_ALS)["gene"].tolist()
features_BA9_ALS = pd.read_csv(lime_file_BA9_ALS)["gene"].tolist()
features_BA4_FTLD = pd.read_csv(lime_file_BA4_FTLD)["gene"].tolist()
features_BA9_FTLD = pd.read_csv(lime_file_BA9_FTLD)["gene"].tolist()

features = list(set(features_BA4_ALS + features_BA9_ALS + features_BA4_FTLD + features_BA9_FTLD))
...
# Map disease status
mapping = {'C9ALS': 1, 'SALS': 2, 'SFTLD': 3, 'C9FTLD': 4, 'PN': 0}
...
## Add comprehensive donor column
adata_pineda_BA9.obs['Donor_comp'] = adata_pineda_BA9.obs["Donor"].astype(str) + "_" + adata_pineda_BA9.obs["Region"].astype(str) + "_" + adata_pineda_BA9.obs["Group"].astype(str)
...
# Map disease status
mapping = {'C9-ALS': 1, 'C9-FTD': 4,  'Control': 0}
...
## Add comprehensive donor column
adata_li_BA9.obs['Donor_comp'] = adata_li_BA9.obs["Donor"].astype(str) + "_" + adata_li_BA9.obs["Region"].astype(str) + "_" + adata_li_BA9.obs["Group"].astype(str)
...
###################################
num_genes = adata_combined_pineda.X.shape[1]
data = []
...
for donor in sample_IDs:
    for _ in range(5):
        ...
        adata_train = adata_combined_pineda[adata_combined_pineda.obs['Donor_comp'] != donor]
        adata_test = adata_combined_pineda[adata_combined_pineda.obs['Donor_comp'] == donor]
        ...
        domains = pd.factorize(adata_train.obs['Donor'])[0] ## Modified
...
out_path = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/generalizable_SALS_C9ALS_SFTLD_C9FTLD_control_Pineda_optimal_gene_set_weighted_LOO_LIME_0_abs_case_control_report_{par_prep}_ALS_{par_keep_cell_type}.csv"
...
X_train = torch.FloatTensor(adata_train_total.X.toarray() if hasattr(adata_train_total.X, 'toarray') else adata_train_total.X)
y_train = torch.LongTensor(adata_train_total.obs['Group'].values)
domains = pd.factorize(adata_train_total.obs['Donor'])[0]
d_train = torch.LongTensor(domains)

dataset = TensorDataset(X_train, y_train, d_train)
input_size = adata_train_total.shape[1]
num_classes = len(np.unique(y_train))
num_domains = len(np.unique(domains))
...
main_model_total_pineda = train_main_total(model_main, model_domain, loader, 15, device)
...
out_path = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/generalizable_SALS_C9ALS_SFTLD_C9FTLD_control_Li_Limone_optimal_gene_set_weighted_LOO_LIME_0_abs_case_control_report_{par_prep}_ALS_{par_keep_cell_type}_unbalanced.csv"
results_df.to_csv(out_path, index=False)
...
largest_non_pineda = (
    adata_combined.obs['Dataset']
    .value_counts()
    .drop('Pineda')
    .max()
)

pineda_mask = adata_combined.obs['Dataset'] == 'Pineda'
...
other_indices = np.where(adata_combined.obs['Dataset'] != 'Pineda')[0]

final_indices = np.concatenate([downsampled_indices, other_indices])

adata_combined_balanced = adata_combined[final_indices].copy()
...
out_path = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/generalizable_SALS_C9ALS_SFTLD_C9FTLD_control_Li_Limone_optimal_gene_set_weighted_LOO_LIME_0_abs_case_control_report_{par_prep}_ALS_{par_keep_cell_type}_balanced_2X.csv"
results_df.to_csv(out_path, index=False)
...
out_path = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/generalizable_SALS_C9ALS_SFTLD_C9FTLD_control_Li_Limone_optimal_gene_set_weighted_LOO_LIME_0_abs_case_control_report_{par_prep}_ALS_{par_keep_cell_type}_balanced_1X.csv"
results_df.to_csv(out_path, index=False)