...
nano SNR_genetic_python_BA9.sh
...
#SBATCH --job-name=SNR_genetic_python_BA9
...
python3.8 /home/fiorini9/scratch/machine_learning_ALS/scripts/SNR_genetic_python_BA9.py

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

nano SNR_genetic_python_BA9.py
...
for par_keep_cell_type in cell_types:
    ...
    # Map disease status
    mapping = {'C9ALS': 2, 'SALS': 1, 'SFTLD': 1, 'C9FTLD': 2,  'PN': 0}
    ...
    ################################### 
    sc.pp.filter_genes(adata_pineda_BA9, min_cells=3)
    sc.pp.normalize_total(adata_pineda_BA9, target_sum=1e4)
    sc.pp.log1p(adata_pineda_BA9)
    sc.pp.highly_variable_genes(adata_pineda_BA9, flavor='seurat')
    
    features = adata_pineda_BA9.var_names[adata_pineda_BA9.var['highly_variable']].tolist()
    len(features)
    ...
    # Map disease status
    mapping = {'C9-ALS': 2, 'C9-FTD': 2,  'Control': 0}
    ...
    # Combine X matrices
    X_combined = vstack([ adata_pineda_BA9.X, adata_li_BA9.X,])
    X_combined.shape
    ...
    obs_combined = pd.concat([adata_pineda_BA9.obs, adata_li_BA9.obs])
    obs_combined.shape[0] == X_combined.shape[0]
    
    var_combined = adata_pineda_BA9.var.copy()
    ...
    ###################################
    # Compute drop out/sparsity
    ###################################  
    def compute_anova_snr_anndata(obj, group_key="Group"):
        """
        Compute ANOVA-style SNR from either:
        - an AnnData object, or
        - a DataFrame with a 'Group' column.
        """
        if hasattr(obj, "X"):  # AnnData case
            expr = pd.DataFrame(
                obj.X.A if hasattr(obj.X, "A") else obj.X,
                index=obj.obs_names,
                columns=obj.var_names
            )
            labels = obj.obs[group_key]
        
        elif isinstance(obj, pd.DataFrame):  # DataFrame case
            expr = obj.drop(columns=[group_key])  # expression = all other columns
            labels = obj[group_key]
    
        else:
            raise ValueError("Input must be AnnData or DataFrame")
    
        # Group by labels
        groups = expr.groupby(labels)
    
        # Compute group means and stds
        group_means = groups.mean()
        group_stds = groups.std()
    
        # Between-group variance of means (signal)
        between_var = group_means.var(axis=0)
    
        # Within-group variance (noise)
        within_var = group_stds.pow(2).mean(axis=0)
    
        # Per-gene SNR
        gene_snr = between_var / within_var.replace(0, np.nan)
    
        # Global SNR
        global_snr = gene_snr.median(skipna=True)
    
        return gene_snr, global_snr
    
    # Make pseudobulk per donor (or sample) first
    combat_cells = pd.DataFrame(
        adata_combined.X.A if hasattr(adata_combined.X, "A") else adata_combined.X,
    ...
    # Add group info
    pseudobulk["Group"] = adata_combined.obs.groupby("Donor_comp")["Group"].first()
    
    # Reuse the earlier ANOVA-SNR function
    gene_snr, global_snr = compute_anova_snr_anndata(pseudobulk, group_key="Group")
    
    print("Per-gene ANOVA-style SNR (first 10):")
    print(gene_snr.head(10))
    
    print("Global SNR (pseudobulk):", global_snr)
...
out_path = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/SNR_genetic_python_BA9.csv"
