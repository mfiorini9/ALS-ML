...
nano LIME_importance_HVG_genetic_BA9_L2_L3.sh
...
for ct in "${celltypes[@]}"; do
    ...
    jobfile="${SCRIPT_DIR}/LIME_importance_HVG_genetic_BA9_${ct}.sh"
    #rm $jobfile
    cat > "$jobfile" <<EOF
#!/bin/bash
...
#SBATCH --mem=100g
#SBATCH --job-name=LIME_importance_HVG_genetic_BA9_${ct}
...
cd ${SCRIPT_DIR}
python3.8 ${SCRIPT_DIR}/LIME_importance_HVG_genetic_BA9_${ct}.py
EOF
    
    # Create the SLURM job script
    pyfile="${SCRIPT_DIR}/LIME_importance_HVG_genetic_BA9_${ct}.py"
    #rm $pyfile
    cat > "$pyfile" <<EOF
...
for donor in sample_IDs:
    ...
    df_index['predicted_label'] = y_all_pred  #y_pred_test_labels_np
    df_index['cell_index'] = df_index.index
    
    disease_status_test = set(y_test)
    
    ## ONLY KEEP CORRECT INDICES. Dont want this. correct indices
    #correct_indices = df_index[df_index['Group'] == df_index['predicted_label']].index
       
    ## convert array to dataframe?
    #X_train_dense = X_train.toarray() if hasattr(X_train, 'toarray') else X_train
    df=pd.DataFrame(X_train,) #X_train_dense
    df_test=pd.DataFrame(X_test,) #X_test_dense
    df_test.index = df_index['cell_index']
    
    ## GET GENE NAMES
    ## For test
    df_test.columns = test_corr.index 
    
    ## For train
    df.columns = train_corr.index 
    ...
    explainer = lime.lime_tabular.LimeTabularExplainer(df.values, class_names=['0', '1', '2'], feature_names=adata_test.var_names.tolist(), verbose=True, mode='classification')
    
    for i in range(num_cells):
        # Convert the NumPy array to a PyTorch tensor
        input_data = torch.tensor(df_test.values[i], dtype=torch.float32).unsqueeze(0)  
...
########################
## Export DNN dataframe
########################
pd.set_option('display.max_rows', None)
results_df = pd.DataFrame(data)
out_path = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/LIME_perfromance_HVG_genetic_BA9_{par_keep_cell_type}_1.csv"
...
###################################
## write csv file
file_name = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/LIME_importance_HVG_genetic_BA9_{par_keep_cell_type}_1.csv"
