...
salloc -A def-grouleau --time=0-8 -c 1 --mem=20g
...
nano LIME_SALS_BA4_Endo_narval_2.sh

#!/bin/bash  
#SBATCH --account=def-sfarhan
#SBATCH --time=01-00:00           # time (DD-HH:MM)
#SBATCH --cpus-per-task=1
#SBATCH --mem=20g          # memory per cor
#SBATCH --job-name=LIME_SALS_BA4_Endo_narval_2
...
python3.8 /home/fiorini9/scratch/machine_learning_ALS/scripts/LIME_SALS_BA4_Endo_narval_2.py

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

nano LIME_SALS_BA4_Endo_narval_2.py
...
def train_and_evaluate(learning_rate, batch_size):
    ...
    # Train the model
    for epoch in range(10):  # Fixed epochs for simplicity
        main_model.train()
        ...
        for X, y in train_loader:
            ...
            class_loss.backward(retain_graph=True)  # Do not retain graph here unless necessary
            main_optimizer.step()
    ...
    with torch.no_grad():
        for X_val, y_val in val_subset:  # Use the last fold's validation set
...
#batch_size = 19
#learning_rate = 0.0009873928150377252
...
def train(main_model, dataloader, num_epochs, device):
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Epoch [{epoch+1}/{num_epochs}], Class Loss: {class_loss.item():.4f}")
...
for fold, (train_idx, val_idx) in enumerate(kf.split(np.arange(len(full_dataset)))):
    ...
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    train(main_model, train_subset, num_epochs=10, device=device)
...
# Train the models
device = 'cuda' if torch.cuda.is_available() else 'cpu'
train(main_model, train_dataloader, num_epochs=10, device=device)
...
## write csv file
file_name = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/model_report_{par_prep}_{par_status}_{par_brain_region}_{par_keep_cell_type}_narval_2.csv"
...
# export to csv file
file_name = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/LIME_index_{par_prep}_{par_status}_{par_brain_region}_{par_keep_cell_type}_narval_2.csv"
...
file_name = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/LIME_{par_prep}_{par_status}_{par_brain_region}_{par_keep_cell_type}_narval_2.csv"
df.to_csv(file_name)
