...
nano HVG_3_class_randomized_BA9_L2_L3.sh
...
for ct in "${celltypes[@]}"; do
    ...
    jobfile="${SCRIPT_DIR}/HVG_3_class_randomized_BA9_${ct}.sh"
    #rm $jobfile
    cat > "$jobfile" <<EOF
#!/bin/bash
...
#SBATCH --mem=100g
#SBATCH --job-name=HVG_3_class_randomized_BA9_${ct}
...
cd ${SCRIPT_DIR}
python3.8 ${SCRIPT_DIR}/HVG_3_class_randomized_BA9_${ct}.py
EOF
    
    # Create the SLURM job script
    pyfile="${SCRIPT_DIR}/HVG_3_class_randomized_BA9_${ct}.py"
    #rm $pyfile
    cat > "$pyfile" <<EOF
...
################################### 
donor_to_group = adata_combined.obs.groupby("Donor_comp")["Group"].first()
shuffled_groups = np.random.permutation(donor_to_group.values)
random_mapping = dict(zip(donor_to_group.index, shuffled_groups))
adata_combined.obs["Group"] = adata_combined.obs["Donor_comp"].map(random_mapping)
...
out_path = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/HVG_3_class_randomized_BA9_{par_keep_cell_type}_3.csv"
