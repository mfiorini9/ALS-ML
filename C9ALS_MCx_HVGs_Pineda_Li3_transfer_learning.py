...
################################### 
adata_combined_pineda = adata_combined[adata_combined.obs['Region'] == "BA4"]
adata_combined_pineda.obs

###################################
# Subset back out Li from the combine object
################################### 
#adata_combined_li = adata_combined[adata_combined.obs['Region'] == "motor cortex"]
#adata_combined_li.obs
...
# LOSO loop
sample_IDs = set(adata_combined_pineda.obs['Donor'])
...
for donor in sample_IDs:
    for _ in range(5):
        ...
        adata_train = adata_combined_pineda[adata_combined_pineda.obs['Donor'] != donor]
        adata_test = adata_combined_pineda[adata_combined_pineda.obs['Donor'] == donor]
...
###################################
# Apply LOSO transfer learning for Li 
###################################
data2 = []
len(set(adata_combined.obs['Donor']))
num_genes = adata_combined.X.shape[1]

sample_IDs_combo = set(adata_combined.obs['Donor'])
#kNN_threshold_list = [.99, .95, .9, .85, .8]
kNN_threshold = 0.9
print(f"KNN Threshold: {kNN_threshold}")
training_epoch = 10
