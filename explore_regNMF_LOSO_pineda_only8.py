...
for par_keep_cell_type in cell_types:
    ...
    class SupervisedNMF(nn.Module):
        def __init__(self, n_cells, n_genes, n_components, lambda_subj=1, lambda_disease=2):
            super().__init__()
            ...
            # 🆕 Small MLP or linear classifier to predict disease from W
        ...
        def forward(self, X, subject_onehot, disease_labels):
            ...
            disease_logits = self.disease_predictor(self.W).squeeze()  # (n_cells,)
            disease_classification_loss = self.bce_loss(disease_logits, disease_labels)
        
            # --- Optional: Keep or remove old disease reward ---
            # (You can comment this out if you want to fully replace it.)
    ...
    sample_IDs = ['210526_FTLD_225_snRNA-C6', '210526_PN_319_snRNA-E5', '210526_PN_303_snRNA-D9', '210526_FTLD_218_snRNA-C3',
                '210526_FTLD_226_snRNA-C7', '210526_FTLD_206_snRNA-B7', '210526_FTLD_204_snRNA-B6', '210526_FTLD_209_snRNA-B9',
                '210526_FTLD_228_snRNA-C8', '210526_FTLD_202_snRNA-B5', '210526_FTLD_217_snRNA-C2', '210526_FTLD_222_snRNA-C4',
    ...
    for donor in sample_IDs:
        ...
        model = SupervisedNMF(n_cells=X.shape[0], n_genes=X.shape[1], n_components=50, lambda_subj=1, lambda_disease=2).to(device)
        ...
        W_np_mean = W_np.mean(axis=0, keepdims=True)
        W_test_np_mean = W_test_np.mean(axis=0, keepdims=True)
       
        W_np_centered = W_np - W_np_mean
        W_test_np_centered = W_test_np - W_test_np_mean
       
        # Now pick same number of points (optional, e.g., randomly subsample W_np)
        # or align using statistics (means, covariances)
       
        # Here we just match second-moments via Procrustes (simple hack):
        min_n = min(W_np_centered.shape[0], W_test_np_centered.shape[0])
       
        W_np_sub = W_np_centered[:min_n]
        W_test_np_sub = W_test_np_centered[:min_n]
       
        # Apply orthogonal Procrustes
        R, _ = orthogonal_procrustes(W_np_sub, W_test_np_sub)
       
        # Rotate W_test_np (the *full* one)
        W_test_np_aligned = (W_test_np - W_test_np_mean) @ R + W_np_mean
       
        # Replace
        W_test_np = W_test_np_aligned.copy()
        W_test = torch.tensor(W_test_np, dtype=torch.float32, device=device)
...
I'll directly patch your code so that it follows your style, and I’ll make minimal changes exactly where needed.

🛠️ Here’s the plan:
Add a small classifier head (self.disease_predictor) to your SupervisedNMF class.

During forward pass:

Predict disease status from W

Compute classification loss (BCEWithLogitsLoss) between predictions and true labels

Modify the final loss to combine:
...
(You can optionally remove the old disease correlation reward — up to you.)

🧩 Here’s your patched version:
python
Copy
Edit
import torch.nn as nn
...
class SupervisedNMF(nn.Module):
    ...
    def forward(self, X, subject_onehot, disease_labels):
        ...
        H_penalty = 1e-4 * torch.norm(self.H, p=2)
...
Before	After
Disease info learned passively via correlation	Disease info learned actively via classification loss
Axis could flip randomly (PN/SFTLD swap)	Axis anchored by classifier (always SFTLD is "positive")
LOSO test cells project inconsistently	LOSO test cells project consistently to the right disease cluster
...
Edit
loss, reconstruction, disease_logits = model(X, subject_onehot, disease_labels)
(just unpack the 3 outputs)

Training otherwise stays the same!
No changes needed to your NNLS / Procrustes / saving steps. ✅

🚀 Why this works:
The NMF W matrix must now both reconstruct and predict disease labels.

So it carves out a disease separation inside the low-dimensional space.

The classifier forces a consistent orientation:

Positive logits → SFTLD
...
🧪 Pro Tip:
You can tweak lambda_disease to control how much emphasis you put on disease classification vs reconstruction:

Higher → stronger disease separation

Lower → better reconstruction, but weaker disease focus
...
(Optional) an even stronger version that makes disease separation orthogonal to subject ID?

(Optional) a version that does Procrustes-free alignment (using shared PCA space)?

Only if you want to take it even further! 🚀
