...
nano generalizable_HVG_L4_L5_ALS_jumbo.sh
...
#SBATCH --job-name=generalizable_HVG_L4_L5_ALS_jumbo
...
python3.8 /home/fiorini9/scratch/machine_learning_ALS/scripts/generalizable_HVG_L4_L5_ALS_jumbo.py

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

nano generalizable_HVG_L4_L5_ALS_jumbo.py
...
remove = ["SFTLD", "C9FTLD"]
remove_li = ['C9-FTD']
par_keep_cell_type =  "L4_L5"


###################################
# Cell Specific parameters -- L4_L5
###################################
...
#ComBat models gene expression = batch effect + (optional) covariates + residual.

#If you include Group as a covariate, ComBat explicitly preserves variation explained by Group.

#It never “uses” the outcome labels in the sense of trying to predict them or align batches by group — it just adjusts batch effects while making sure variation due to Group remains.

#Why I said “not catastrophic”

#If ComBat were to directly use your labels (Group) to align samples by disease status, that would be true target leakage: your downstream DNN would be seeing a representation already influenced by the very signal it’s supposed to learn.

#But ComBat doesn’t do that. It just ensures batch effects don’t wipe out Group variation. So even though the test donor contributes to parameter estimation, the preserved Group variation is not artificially inflated — it’s just not removed.

#🔑 So to restate more clearly:

#By “labels,” I meant your Group variable (disease vs control).

#ComBat doesn’t use these labels in a predictive sense — it just protects them from being corrected away.
...
out_path = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/generalizable_Pineda_optimal_gene_set_weighted_LOO_LIME_0_abs_case_control_report_{par_prep}_ALS_{par_keep_cell_type}.csv"
...
out_path = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/generalizable_Li_Limone_optimal_gene_set_weighted_LOO_LIME_0_abs_case_control_report_{par_prep}_ALS_{par_keep_cell_type}_unbalanced.csv"
results_df.to_csv(out_path, index=False)
...
out_path = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/generalizable_Li_Limone_optimal_gene_set_weighted_LOO_LIME_0_abs_case_control_report_{par_prep}_ALS_{par_keep_cell_type}_balanced_2X.csv"
results_df.to_csv(out_path, index=False)
...
out_path = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/generalizable_Li_Limone_optimal_gene_set_weighted_LOO_LIME_0_abs_case_control_report_{par_prep}_ALS_{par_keep_cell_type}_balanced_1X.csv"
results_df.to_csv(out_path, index=False)