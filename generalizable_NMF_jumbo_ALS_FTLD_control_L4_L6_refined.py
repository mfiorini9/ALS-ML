...
nano generalizable_NMF_L3_L5_ALSand_FTLD_jumbo.sh
...
#SBATCH --job-name=generalizable_NMF_L3_L5_ALSand_FTLD_jumbo
...
python3.8 /home/fiorini9/scratch/machine_learning_ALS/scripts/generalizable_NMF_L3_L5_ALSand_FTLD_jumbo.py

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

nano generalizable_NMF_L3_L5_ALSand_FTLD_jumbo.py
...
## Balance the counts -- well keep 2X control
#group_col = 'Group'
...
desired_ratio = {1: 1, 2: 1, 0: 2}

# Count available cells in each group
counts = adata_combined_pineda.obs[group_col].value_counts().to_dict()

# Compute the scaling factor: smallest ratio that still fits all groups
scales = {g: counts[g] // desired_ratio[g] for g in desired_ratio}
max_k = min(scales.values())

# Compute final target sizes
target_sizes = {g: desired_ratio[g] * max_k for g in desired_ratio}
print("Target sizes:", target_sizes)

# Sample indices
selected_idx = []
for group, n_target in target_sizes.items():
    group_idx = np.where(adata_combined_pineda.obs[group_col] == group)[0]
    sampled = np.random.choice(group_idx, n_target, replace=False)
    selected_idx.extend(sampled)

# Subset AnnData
adata_balanced = adata_combined_pineda[selected_idx].copy()

# Check final distribution
print(adata_balanced.obs[group_col].value_counts())


# We should downsample pineda -- takes wayyy too long
if adata_balanced.n_obs > 10000:
    random_indices = np.random.choice(adata_balanced.n_obs, size=10000, replace=False)
...
out_path = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/ALS_and_FTLD_generalizable_NMF_Pineda_optimal_gene_set_weighted_LOO_LIME_0_abs_case_control_report_{par_prep}_ALS_{par_keep_cell_type}.csv"
...
main_model_total_pineda = train_main_total(model_main, model_domain, loader, 30, device)
...
out_path = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/ALS_and_FTLD_generalizable_NMF_Li_Limone_optimal_gene_set_weighted_LOO_LIME_0_abs_case_control_report_{par_prep}_ALS_{par_keep_cell_type}_unbalanced.csv"
results_df.to_csv(out_path, index=False)
...
out_path = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/ALS_and_FTLD_generalizable_NMF_Li_Limone_optimal_gene_set_weighted_LOO_LIME_0_abs_case_control_report_{par_prep}_ALS_{par_keep_cell_type}_balanced_1X.csv"
results_df.to_csv(out_path, index=False)
...
out_path = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/ALS_and_FTLD_generalizable_NMF_Li_Limone_optimal_gene_set_weighted_LOO_LIME_0_abs_case_control_report_{par_prep}_ALS_{par_keep_cell_type}_balanced_2X.csv"
results_df.to_csv(out_path, index=False)
