...
nano NMF_diagnosis_BA4_cell_true_label_L2_L3.sh
...
for ct in "${celltypes[@]}"; do
    ...
    jobfile="${SCRIPT_DIR}/NMF_diagnosis_BA4_cell_true_label_${ct}.sh"
    rm $jobfile
    cat > "$jobfile" <<EOF
#!/bin/bash
...
#SBATCH --mem=100g
#SBATCH --job-name=NMF_diagnosis_BA4_cell_true_label_${ct}
...
cd ${SCRIPT_DIR}
python3.8 ${SCRIPT_DIR}/NMF_diagnosis_BA4_cell_true_label_${ct}.py
EOF
    
    # Create the SLURM job script
    pyfile="${SCRIPT_DIR}/NMF_diagnosis_BA4_cell_true_label_${ct}.py"
    rm $pyfile
    cat > "$pyfile" <<EOF
...
out_path = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/NMF_diagnosis_BA4_cell_true_label_{par_keep_cell_type}_3.csv"
