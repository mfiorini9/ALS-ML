salloc -A def-grouleau --time=0-2 -c 1 --mem=200g
...
nano test_SFTLD_BA9_regNMF11_narval_2.sh
...
#SBATCH --job-name=test_SFTLD_BA9_regNMF11_narval_2
...
python3.8 /home/fiorini9/scratch/machine_learning_ALS/scripts/test_SFTLD_BA9_regNMF11_narval_2.py

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

nano test_SFTLD_BA9_regNMF11_narval_2.py
...
for par_keep_cell_type in cell_types:
    ...
    # --- SupervisedNMFEncoder ---
     
    class SupervisedNMFEncoder(nn.Module):
        def __init__(self, n_genes, n_components, n_domains, lambda_subj=0.1, lambda_disease=50, lambda_domain=1):
            super().__init__()
            ...
            self.domain_classifier = nn.Linear(n_components, n_domains)
            self.bce_loss = nn.BCEWithLogitsLoss()
            self.ce_loss = nn.CrossEntropyLoss()
     
        def forward(self, X, subject_onehot, disease_labels):
            W = self.encoder(X)
            reconstruction = W @ self.H
     
            # Reconstruction loss
            recon_loss = F.mse_loss(reconstruction, X)
     
            # Subject decorrelation
            W_norm = W - W.mean(dim=0, keepdim=True)
            subj_norm = subject_onehot - subject_onehot.mean(dim=0, keepdim=True)
            eps = 1e-8
            subj_corr = torch.matmul(W_norm.T, subj_norm) / (
                (torch.norm(W_norm, dim=0).unsqueeze(1) + eps) * (torch.norm(subj_norm, dim=0) + eps)
            )
            subject_penalty = torch.mean(torch.abs(subj_corr))
     
            # Disease classification
            disease_logits = self.disease_predictor(W).squeeze()
            disease_classification_loss = self.bce_loss(disease_logits, disease_labels)
     
            # Domain adversarial loss
            rev_features = grad_reverse(W, lambd=1.0)
            domain_logits = self.domain_classifier(rev_features)
            domain_labels = subject_onehot.argmax(dim=1)
            domain_classification_loss = self.ce_loss(domain_logits, domain_labels)
            ...
            return loss, reconstruction, disease_logits, W
    ...
    for donor in sample_IDs:
        ...
        X_train = torch.tensor(adata_train.X.toarray(), dtype=torch.float32, device=device)
        ...
        model = SupervisedNMFEncoder(
            n_genes=X_train.shape[1],
            n_components=30,
            n_domains=subject_onehot.shape[1],
            lambda_subj=0.1,
            lambda_disease=50,  # boost disease signal
            lambda_domain=1
        ).to(device)
        ...
        for epoch in range(200):
            optimizer.zero_grad()
            loss, reconstruction, disease_logits, W_train = model(X_train, subject_onehot, disease_labels)
            loss.backward()
            optimizer.step()
     
            model.H.data.clamp_(min=0)
        ...
        # --- Project test set directly through encoder ---
        with torch.no_grad():
            W_test = model.encoder(X_test)
     
        # --- Save latent components into new AnnData objects ---
        W_np = W_train.detach().cpu().numpy()
        ...
        plot_latent_components_to_pdf(
            W_train, disease_labels, num_components=10,
    ...
    out_path = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/regNMF_model_report11_{par_prep}_{par_status}_{par_brain_region}_{par_keep_cell_type}_narval_2.csv"
