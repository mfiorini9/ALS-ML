...
nano Drop_out_sparsity_python_BA9.sh
...
#SBATCH --job-name=Drop_out_sparsity_python_BA9
...
python3.8 /home/fiorini9/scratch/machine_learning_ALS/scripts/Drop_out_sparsity_python_BA9.py

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

nano Drop_out_sparsity_python_BA9.py
...
for par_keep_cell_type in cell_types:
    ...
    ###################################  
    # Compute fraction of zeros per cell (sparsity)
    if isinstance(adata_combined.X, np.ndarray):
        sparsity_per_cell = (adata_combined.X == 0).mean(axis=1)  # axis=1: fraction of zeros per cell
    else:
        sparsity_per_cell = (adata_combined.X == 0).mean(axis=1).A1  # handle sparse matrix
    
    # Summarize across all cells (median sparsity)
    median_sparsity = np.median(sparsity_per_cell)
    
    print(f"Median sparsity across all cells: {median_sparsity:.3f}")
...
out_path = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/Drop_out_sparsity_python_BA9.csv"
