...
nano HVG_disease_BA4_cell_true_label_No_LiFTD_L2_L3.sh
...
for ct in "${celltypes[@]}"; do
    ...
    jobfile="${SCRIPT_DIR}/HVG_disease_BA4_cell_true_label_No_LiFTD_${ct}.sh"
    rm $jobfile
    cat > "$jobfile" <<EOF
#!/bin/bash
...
#SBATCH --mem=100g
#SBATCH --job-name=HVG_disease_BA4_cell_true_label_No_LiFTD_${ct}
...
cd ${SCRIPT_DIR}
python3.8 ${SCRIPT_DIR}/HVG_disease_BA4_cell_true_label_No_LiFTD_${ct}.py
EOF
    
    # Create the SLURM job script
    pyfile="${SCRIPT_DIR}/HVG_disease_BA4_cell_true_label_No_LiFTD_${ct}.py"
    rm $pyfile
    cat > "$pyfile" <<EOF
...
adata_li_BA4 = adata_li_BA4[adata_li_BA4.obs['Group'] != 4].copy()
...
out_path = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/HVG_disease_BA4_cell_true_label_No_LiFTD_{par_keep_cell_type}_3.csv"
