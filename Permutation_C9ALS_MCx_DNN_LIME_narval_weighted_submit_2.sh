...
# List of cell types
CELL_LIST=( "L3_L5" "L2_L3" "L4_L6" "L4_L5" "L5_L6" "L6" "PV" "5HT3aR" "Rosehip" "SOM"  "Oligo" "Astro" "OPC" "Micro" "T_Cell" "Mural" "Endo" "Fibro" "L5")

generate_py_file() {
    CELL=$1
     
    # Start with the template content, replacing the cell variable
    rm "/home/fiorini9/scratch/machine_learning_ALS/scripts/permutation_weighted_LIME_C9ALS_BA4_${CELL}_narval_2.py"
    
    cat <<EOF > "/home/fiorini9/scratch/machine_learning_ALS/scripts/permutation_weighted_LIME_C9ALS_BA4_${CELL}_narval_2.py"
## generalizable model
...
file_name = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/pineda_LIME_weighted_percentile_rank_{par_prep}_{par_status}_{par_brain_region}_{par_keep_cell_type}_narval_2.csv"
df.to_csv(file_name, index=False)


###################################
# Cell Specific parameters -- ${CELL}
###################################
data = []
...
## write csv file
file_name = f"/home/fiorini9/scratch/machine_learning_ALS/model_outs/permutation_weighted_report_{par_prep}_{par_status}_{par_brain_region}_{par_keep_cell_type}_narval_2.csv"
df.to_csv(file_name, index=False)

EOF

    echo "Py file created for cell type : $CELL at /home/fiorini9/scratch/machine_learning_ALS/scripts/permutation_weighted_LIME_C9ALS_BA4_${CELL}_narval_2.py"
}

# Loop through the pool list and generate the .ini file for each
for CELL in "${CELL_LIST[@]}"; do
    generate_py_file "$CELL"
done
...
########################################################################################################
########################################################################################################
######################################################################################################## generate and submit bash script
...
# Create a function to generate the ensemblex_config.ini file
generate_and_submit_job() {
    CELL=$1

    # Generate the job script
    cat <<EOF > "/home/fiorini9/scratch/machine_learning_ALS/scripts/permutation_weighted_LIME_C9ALS_BA4_${CELL}_narval_2.sh"
#!/bin/bash  
#SBATCH --account=def-grouleau
#SBATCH --time=07-00:00           # time (DD-HH:MM)
#SBATCH --cpus-per-task=1
#SBATCH --mem=40g          # memory per cor
#SBATCH --job-name=permutation_weighted_LIME_C9ALS_BA4_${CELL}_narval_2
...
python3.8 /home/fiorini9/scratch/machine_learning_ALS/scripts/permutation_weighted_LIME_C9ALS_BA4_${CELL}_narval_2.py

EOF
    
    # Submit the job using sbatch and capture the job ID
    JOB_SUBMISSION_OUTPUT=$(sbatch "/home/fiorini9/scratch/machine_learning_ALS/scripts/permutation_weighted_LIME_C9ALS_BA4_${CELL}_narval_2.sh")

    # Extract the job ID from the sbatch output
    JOB_ID=$(echo "$JOB_SUBMISSION_OUTPUT" | awk '{print $4}')

    # Print the job submission information
    echo "Job submitted for cell type: $CELL with job ID: $JOB_ID"
}

# Loop through the pool list and generate the job script for each pool, then submit the job
for CELL in "${CELL_LIST[@]}"; do
    generate_and_submit_job "$CELL"
done
